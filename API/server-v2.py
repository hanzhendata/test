...
def Sche_Init():
    ...
    job_last  = scheduler.add_job(func=WarningMessageAll(), trigger='cron', day_of_week='0-6',hour='15',minute=35 ,replace_existing=True)
    scheduler.resume()
    # scheduler.start()
...
def Days_Init():
    ...
    if len(stocksOHLC) == 0 :
        ...
        cursor = Connector.cursor()
        sql = "select open,high,low,close,date from stocks_history_days where ktype=7 and windcode=%(code)s and date>%(date)s order by date desc limit 30"
...
def OHLC_RemoveLast():
    ...
    for windcode in  stocksOHLC :
        ...
        if lastdate.hour == 9 or lastdate.hour == 13 :       
...
def WarningMessage_Init(GdbbFlag=False):
    ...
    for bname,windcode,stype in cursor:
        ...
        if windcode in Windcode_Dict :
            Windcode_Dict [windcode].update({'name':bname,'type':stype,'wsq2':0,'wsq5':0,'wsq6':0,} ) 
        else:
            Windcode_Dict [windcode] = {'name':bname,'type':stype,'wsq2':0,'wsq5':0,'wsq6':0,}
        ...
        for index in range(len( wd[wi] )):
            ...
            if wi==1: #stocksMainForce satify either in boardstrong or in boardready
                if not stocksLabel[windcode]['is_bs'] and not stocksLabel[windcode]['is_br'] :
                    Windcode_Dict.pop(windcode,None)
                    continue
            if Windcode_Dict.get(windcode) is None:                
                Windcode_Dict [windcode] = {'name':wd[wi][index]['name'],'type':wi+4,'wsq2':0,'wsq5':0,'wsq6':0,}
                Windcode_Dict[windcode].update(stocksLabel[ windcode] )
    cursor.close()
    ...
    for ktype in ktype_list :
        ...
        for windcode in  Windcode_Dict  :
            ...
            if ktype== 6 or ktype==8 or not GdbbFlag :        
                continue
...
def WarningMessage_SignalRebuild():
    MarketType = 1
    sql = "select  name, windcode,stype,date from stocks_sr where date"   
    sql_w = "select wtype,ktype,date,stype  from stocks_warning where windcode=%(code)s  order by date desc limit 1"
    cursor = Connector.cursor()
    cursor_w = Connector.cursor()
    num = 0
    ...
    for bname,windcode,stype,sdate in cursor:
        if windcode in stocksSuspend0:
            continue  
        Windcode_Dict [windcode] = {'name':bname,'type':stype,'date':sdate,'wsq2':0,'wsq5':0,'wsq6':0,}
        Windcode_Dict[windcode].update(stocksLabel[ windcode] )
    print len(Windcode_Dict),len(stocksBasicSelect),len(stocksMainForce),len(stocksEventDriven)
    wd=[stocksBasicSelect,stocksMainForce,stocksEventDriven]
    for wi in range(len(wd)):
    ...
    for ktype in [6] :       
        ...
        for windcode in  Windcode_Dict  :
            ...
            for index in range( len(['tech'+ ktypestr]) ) :
                wtc=WindPy.tdayscount(Windcode_Dict[windcode][ktypestr][-index-1]['date'],Windcode_Dict[windcode][ktypestr][-1]['date'], "")
                if wtc.ErrorCode ==0 :
                    if wtc.Data[0][0] > 10 :
                        break
                if ( Windcode_Dict[windcode]['type']==2 and Windcode_Dict[windcode]['tech'+ktypestr]['fast_j'][-index-1] < 5 ) or ( ktype==6 and Windcode_Dict[windcode]['type']!=2 and Windcode_Dict[windcode]['tech'+ktypestr]['slow_j'][-index-1] < 5 ):
                        print windcode,ktype,Windcode_Dict[windcode][ktypestr][-index-1]['date']
                        Windcode_Dict[windcode].update({'warning_ktype':ktype,'warning_status':10,'wdate':Windcode_Dict[windcode][ktypestr][-index-1]['date']}) 
    ...
    for ktype in [2,5]:
        for windcode in Windcode_Dict:
            ...
            if ktype == 2 :
                for i in range(-index-1,0):
                    if  FourIndicator.GoldenDead(Windcode_Dict[windcode]['tech'+ktypestr][i-1:]) == 4 :                        
                        Windcode_Dict[windcode].update({'warning_status':40,'warning_ktype':ktype,'wdate':Windcode_Dict[windcode][ktypestr][i]['date']})
                        break
            if ktype==5:
                for i in range(-index-1,0):
                    # print -index-1,i,indicator[i]['date']
                    ...
                    if gl==1 and Windcode_Dict[windcode]['is_strong'] :
                        ws = 40
                        Windcode_Dict[windcode].update({'warning_status':ws,'warning_ktype':ktype,'wdate':Windcode_Dict[windcode][ktypestr][i]['date']})               
                        break
                    if gl==2 :
...
def WarningMessage2(LastFlag=False):
    ...
    for windcode in Windcode_Dict:
        if Windcode_Dict[windcode]['type'] != 2:
            continue
        if LastFlag:            
            wsi=Slice.SliceMinute_WindPy(windcode,5,WindPy,current_time.replace(hour=14,minute=56,second=0),current_time.replace(hour=15,minute=1,second=0))
    ...
    for windcode in Windcode_Dict:        
        ...
        if Windcode_Dict[windcode]['wsq'+ktypestr] >1:
            ...
            wsi=Slice.SliceMinute_WindPy(windcode,5,WindPy,temp['date']-timedelta(minutes=9),temp['date']-timedelta(minutes=4))
            if wsi is not None:
                temp.update(wsi)
...
def WarningMessage(ktype,LastFlag=False):
    ...
    for windcode in Windcode_Dict:
        ...
        if LastFlag:            
            ...
            if not Stock.CheckWindData(wsi,Message='WSQ Error') :
                ...
                dtime = datetime.strptime(str(int(wsi.Data[0][0] )),'%H%M%S')
                temp = {
                    'date':  current_time.replace(second=0 , microsecond = 0,
                        hour= dtime.hour, minute=dtime.minute- dtime.minute % 30),
                    'open':  wsi.Data[1][0] ,
                    'high':  wsi.Data[2][0],
                    'low':   wsi.Data[3][0],
                    'close': wsi.Data[4][0] ,
    ...
    for windcode in Windcode_Dict:        
        ...
        if Windcode_Dict[windcode]['wsq'+ktypestr] >1:
            temp=Windcode_Dict[windcode][ktypestr][-2]
            wsi=Slice.SliceMinute_WindPy(windcode,BarSize,WindPy,temp['date']-timedelta(minutes=2*BarSize-1),temp['date']-timedelta(minutes=BarSize-1))
            if wsi is not None:
                temp.update(wsi)
                last=Windcode_Dict[windcode][ktypestr].pop()
                Windcode_Dict[windcode][ktypestr].pop()
        ...
        Windcode_Dict[windcode]['8'].append(temp) 
        Windcode_Dict[windcode]['tech8'] = Stock.StocksTech(Windcode_Dict[windcode]['8'])
        if Windcode_Dict[windcode]['type']<4 and Windcode_Dict[windcode]['tech8']['fast_k'][-1] <=Windcode_Dict[windcode]['tech8']['fast_d'][-1]:
            continue
        Windcode_Dict[windcode]['tech'+ktypestr] = Stock.StocksTech(Windcode_Dict[windcode][ktypestr]) 
...
if __name__ == '__main__':
    ...
    if Debug:        
        EventDrivenStocks()
        Days_Init()
        print stocksOHLC['600123.SH']['update_date'][-1]
        OHLC_Update()
        print stocksOHLC['600123.SH']['update_date'][-1]
        OHLC_RemoveLast()
        print stocksOHLC['600123.SH']['update_date'][-1]
