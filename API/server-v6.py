...
def Sche_Init():
    ...
    s10 = current_date.replace(hour= 9, minute=30,second= 30)
    ...
    e10 = current_date.replace(hour= 9,minute=32, second= 20)
    s20 = current_date.replace(hour=13,minute=0, second= 30)
    ...
    for windcode in Windcode_Dict:
        ...
        if cursor_w.rowcount>0:
            ...
            if ts.ErrorCode!=0 or \
              ( Windcode_Dict[windcode]['is_bc'] and ts.Data[0][0] >Config.BreakCentralPermitDays) or \
              ( Windcode_Dict[windcode]['is_bv'] and ts.Data[0][0] >Config.BreakVolumePermitDays):
...
def WarningMessage_SignalRebuild(current_datetime,SendWM=False):
    ...
    gdate = gd.Data[0][0]
    MarketType = 1
    # sql = "select  name, windcode,stype,date from stocks_sr where windcode='002402.SZ' order by windcode asc,date desc"   
    ...
    cursor = Connector.cursor()
    sql = "Insert into stocks_warning(id,wtype,stype,ktype,windcode,name,price,date) values(%(sid)s,%(w)s,%(s)s,%(k)s,%(code)s,%(name)s,%(price)s,%(wdate)s)"
    f =open('Signal.txt','w')
    stocksWM= []
    for windcode in  Windcode_Dict  :
        tablename = 'stocks_history_minutes'
        for ktype in [6] :
            ...
            for index in range( len(Windcode_Dict[windcode][ktypestr]) ) :
                ...
                if Windcode_Dict[windcode][ktypestr][-index-1]['date'] <= Windcode_Dict[windcode]['date'] :
                    break
                print Windcode_Dict[windcode][ktypestr][-index-1]['date'],-index-1,Windcode_Dict[windcode]['tech'+ktypestr]['slow_j'][-index-1]
                if ( Windcode_Dict[windcode]['is_bc'] and Windcode_Dict[windcode]['tech'+ktypestr]['fast_j'][-index-1] < 5 ) or ( not Windcode_Dict[windcode]['is_bc'] and Windcode_Dict[windcode]['tech'+ktypestr]['slow_j'][-index-1] < 5 ):
                        ...
                        if Windcode_Dict[windcode]['is_bc']:
                            Windcode_Dict[windcode].update({'type':2}) 
                        Windcode_Dict[windcode].update({'warning_ktype':ktype,'warning_status':10,'warning_date':Windcode_Dict[windcode][ktypestr][-index-1]['date'],'warning_price':Windcode_Dict[windcode][ktypestr][-index-1]['close']}) 
        
        if Windcode_Dict[windcode]['warning_status'] != 10 :
            continue
        f.write(" code %s date %s type %d Signal %d ktype %d\n" %(windcode,Windcode_Dict[windcode]['warning_date'].strftime("%Y-%m-%d %H:%M:%S")
            ,Windcode_Dict[windcode]['type'] ,Windcode_Dict[windcode]['warning_status']/10,Windcode_Dict[windcode]['warning_ktype']))
        # if Windcode_Dict[windcode]['warning_date'].strftime('%Y-%m-%d')  == current_datetime.strftime('%Y-%m-%d') or Windcode_Dict[windcode]['warning_date'].strftime('%Y-%m-%d')  == (current_datetime-timedelta(days=1)).strftime('%Y-%m-%d') or         Windcode_Dict[windcode]['warning_date'].strftime('%Y-%m-%d')  == (current_datetime-timedelta(days=2)).strftime('%Y-%m-%d'):
        if SendWM and Windcode_Dict[windcode]['warning_date'].strftime('%Y-%m-%d')  == current_datetime.strftime('%Y-%m-%d'):
            stocksWM.append( {
                        'id': 100,
                        'windcode': windcode,
                        'name' : Windcode_Dict[windcode]['name'],
                        'warning_status': Windcode_Dict[windcode]['warning_status']/10,
                        'type' : Windcode_Dict[windcode]['type'],
                        'update_date' : Windcode_Dict[windcode]['warning_date'].strftime("%Y-%m-%d %H:%M:%S"),

                        'price'   : str(Windcode_Dict[windcode]['warning_price']) ,
        ...
        # if SendWM :       
            cursor.execute(sql,{
                ...
                'name':Windcode_Dict[windcode]['name'],
                'code':windcode,
                'price': Windcode_Dict[windcode]['warning_price'],
                'wdate': Windcode_Dict[windcode]['warning_date']    
                })
        for ktype in [2,5]:
            ktypestr = str(ktype) 
            ...
            if ktype == 2 :
                for i in range(-index,0):
                    ...
                    if Windcode_Dict[windcode]['warning_status'] != 40 and FourIndicator.GoldenDead(temp) == 4 :                        
                        Windcode_Dict[windcode].update({'warning_status':40,'warning_ktype':ktype,'warning_date':Windcode_Dict[windcode][ktypestr][i]['date'],'warning_price':Windcode_Dict[windcode][ktypestr][i]['close']})
                        break
            if ktype == 5 :
                for i in range(-index,0):
                    # print -index-1,i,indicator[i]['date']
                    ...
                    if gl==1 and Windcode_Dict[windcode]['is_strong'] :
                        ws = 40
                        Windcode_Dict[windcode].update({'warning_status':ws,'warning_ktype':ktype,'warning_date':Windcode_Dict[windcode][ktypestr][i]['date'],'warning_price':Windcode_Dict[windcode][ktypestr][i]['close']})               
                        break
                    if gl==2 :
...
def WarningMessageAll():
    if not TradeDayFlag :
        print "WarningMessageAll Error! not TradeDay!"
        return
...
def WarningMessage(ktype,LastFlag=False):
    ...
    for windcode in Windcode_Dict:        
        ...
        if ktype ==  5 :
            ...
            if  Windcode_Dict[windcode]['is_strong'] :
                if  gl==1 :
                    ws = 4
...
if __name__ == '__main__':
    ...
    if Debug:
        ...
        # EventDrivenStocks()       
        Days_Init()
        WarningMessage_SignalRebuild(datetime.now()-timedelta(days=1),SendWM=True)
