...
def Sche_Init():
    ...
    s10 = current_date.replace(hour= 9, minute=31,second= 30)
...
def UserStocks():
    global stocksUser
    url = "https://www.hanzhendata.com/ihanzhendata/stock/user_stock"
    response = requests.request("GET", url)
    if response.status_code == 200 :
        data = response.json()
        # print 'EventDrivenStocks ',response.status_code,data
...
def WarningMessage_Init(GdbbFlag=False):
    ...
    for bname,windcode,stype in cursor:
        if windcode in stocksSuspend0:
            continue  
        Windcode_Stype[stype][windcode] = {'name':bname,'wsq2':0,'wsq5':0,'wsq6':0,}
...
def WarningMessage_SignalRebuild(current_datetime,SendWM=False):
    ...
    for uid in stocksUser:
        for windcode in stocksUser[uid]:
            if windcode not in stocksLabel :### 510500.SH
                continue
            if Wd_Rb.get(windcode) is None:
                Wd_Rb [windcode] = {'warning_status':4,'type':[],'date':[],'wsq2':0,'wsq5':0,'wsq6':0,}
                Wd_Rb[windcode].update(stocksLabel[ windcode] )
    ...
    #             Wd_Rb[windcode].update({'warning_status':0,})
    #             continue
    #         ws = temp[0]            
    #         Wd_Rb[windcode].update({'warning_status':ws,})            
    #         if ws==1 :
    #             Wd_Rb[windcode].update({'warning_status':10,'warning_ktype':temp[1],'warning_date':temp[2],})            
    #     else:
    #         Wd_Rb[windcode].update({'warning_status':0,})
    #     num += 1
    #     # if num>6:
            ...
            if ktype == 8 :
                ...
                if len(last_date)>0 and board[ds][-1]['date'].date() < last_date[-1]:
                    ...
                    if last_data is not None and len(last_data) > 0: 
                        ...
                        Wd_Rb[windcode][ds].append(last_data) 
                board.pop('ta'+str(ktype),None)  
                board.pop('ta'+str(ktype-1),None)
    ...
    for windcode in  Wd_Rb  :
        ...
        for wi in range( wdlen ):
            ...
            try:
                tdo = WindPy.tdaysoffset(1,Wd_Rb[windcode]['date'][wi],"")
                tdo= tdo.Data[0][0]
            except Exception, e:
                tdo = Wd_Rb[windcode]['date'][wi]+timedelta(days=1)
                ...
                for ktype in [2,5,6]:
                    ...
                    for index in range( len(Wd_Rb[windcode][ktypestr]) ) :
                        ...
                        if dtime>Wd_Rb[windcode]['date'][wi] and dtime>Wd_Rb[windcode]["warning_date"] and dtime < tdo :
                            ...
                            if ktype == 6 :
                                if Wd_Rb[windcode]['warning_status']==40 :                                    
                    ...
                    if ws ==0 :
                        continue
                    wo = num
                    if Wd_Rb[windcode].get('warning_order'):
                        wo = Wd_Rb[windcode].get('warning_order')
                    ...
                    if SendWM and Wd_Rb[windcode]['warning_date'].date() == cal_date.date():
...
def WarningMessage(ktype,LastFlag=False):
    ...
    for windcode in Windcode_Dict:        
        ...
        for index in index_list :    
            if Windcode_Stype[index].get(windcode) is None :
                continue
            if Windcode_Stype[index][windcode]['wsq'+ktypestr] ==0 :
                continue
            W= Windcode_Dict[windcode][ktypestr][ -Windcode_Stype[index][windcode]['wsq'+ktypestr]: ]
            ...
            if Windcode_Stype[index][windcode]['warning_status']==0 :
                ...
                if ktype ==  5 :
                    if Windcode_Stype[index][windcode]['warning_status']==4:
                        # print index,windcode,Windcode_Stype[index][windcode]
...
logging.basicConfig(level=logging.INFO, 
        ...
        datefmt='%Y-%m-%d %H:%M:%S',  filename='log_7.txt',  filemode='a')
clients = set()
...
if __name__ == '__main__':
    ...
    job_minit = scheduler.add_job(func=OHLC_Update, trigger='cron', day_of_week='0-6',hour='9',minute='33',replace_existing=True)
    ...
    if Debug:                
        ...
        WarningMessage_SignalRebuild(dt,SendWM=SendWMFlag)
